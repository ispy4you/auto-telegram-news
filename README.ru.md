...
редактирования и подтверждения постов на каждом шаге.
...
- [Запуск](#запуск)
- [Работа в админке](#работа-в-админке)
...
### Сбор контента
- Читает посты из любого числа публичных Telegram-каналов через **Telethon user session** — без
  необходимости быть администратором источника.
- **Режим реального времени**: постоянное Telethon-соединение с обработчиком `NewMessage` — новый
  пост сохраняется мгновенно, без ожидания следующего тика планировщика.
- **Автоматический catch-up**: при старте (или переподключении) подтягиваются пропущенные
  сообщения по каждому каналу.
- **Polling как fallback**: планировщик продолжает работать при потере соединения; когда event
  listener активен — шаг fetch пропускается автоматически.
...
- Отсечка по возрасту при сборе (по умолчанию 24 ч, настраивается): после долгого простоя старый
  бэклог пропускается, а не заваливает пайплайн неактуальными новостями.
- Скачивает и сохраняет вложения: фото, видео, документы; корректная обработка альбомов (grouped
  messages).
...
- **fastembed** (`sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2`) — семантическая
  дедупликация: ловит смысловые дубликаты, которые rapidfuzz пропускает («Путин подписал закон» ≈
  «Президент подписал документ»). Порог и включение настраиваются в UI; модель ~220 МБ, загружается
  один раз.
...
### AI-генерация
- Интеграция с **Timeweb AI Gateway** (OpenAI-совместимый endpoint) — легко подключить любой
  OpenAI-совместимый API.
...
- **Предпросмотр в стиле Telegram** прямо на странице поста: пузырь сообщения, медиагрид, счётчик
  символов, горячая клавиша `Ctrl+P` — обновляется в реальном времени при редактировании.
...
- Маршруты: задаёте, какой источник идёт в какой канал; если маршрутов нет — посты рассылаются во
  все активные каналы.
...
- **Расписание публикаций**: для каждого целевого канала задаётся временно́е окно (`publish_from` /
  `publish_to`). Посты вне окна уходят в очередь и публикуются автоматически при открытии
  следующего окна.
- `telegram_message_id` сохраняется после отправки — для будущего редактирования и аналитики.

### Мультипроектность
- Неограниченное число **проектов** — изолированные пространства с собственными источниками,
  каналами и маршрутами.
- Быстрое переключение через выпадающее меню в шапке; все счётчики и списки фильтруются по
  текущему проекту.
...
| AI | httpx + OpenAI-совместимый endpoint |
...
Самый быстрый способ попробовать — не нужно ставить Python/PostgreSQL, только Docker.

```bash
git clone https://github.com/ispy4you/auto-telegram-news.git
cd auto-telegram-news
cp .env.example .env
# отредактируйте .env: задайте APP_SECRET_KEY, ADMIN_PASSWORD, а также (опционально, чтобы сразу
# включить сбор/AI/публикацию) TELEGRAM_API_ID/HASH, TELEGRAM_BOT_TOKEN, TIMEWEB_AI_GATEWAY_*

docker compose up -d --build
```

Это поднимет и приложение, и PostgreSQL (вторым контейнером); таблицы создаются автоматически.
Откройте **http://localhost:8000/login**, затем перейдите в **Settings → Telegram-аккаунт** и
войдите через QR-код или телефон+код — прямо в браузере, без доступа к терминалу. Сессия
сохраняется в `./data/telegram_session/` на хосте (примонтирована в контейнер), поэтому переживает
пересборки и рестарты.

По умолчанию база использует `POSTGRES_USER`/`POSTGRES_PASSWORD`/`POSTGRES_DB` из `.env` (если не
заданы — `tgnews`/`tgnews`/`tgnews`) — задайте реальный пароль там, если это не просто локальный
тест, поскольку `docker-compose.yml` автоматически собирает `DATABASE_URL` из этих переменных.

Остановить: `docker compose down` (добавьте `-v`, чтобы также удалить том с базой данных).

### Деплой в Timeweb Cloud (хостинг, оплата в рублях)

Хотите запустить не на своей машине, а Railway/Render/Fly.io не подходят, потому что требуют
не-российскую карту? [Timeweb Cloud](https://timeweb.cloud) принимает оплату в рублях/СБП/Мир и
хорошо подходит для этого проекта.

**Вариант 1: облачный сервер (VPS) — рекомендуется.** Обычный VPS запускает `docker-compose.yml`
без изменений, включая примонтированные тома, благодаря которым Telegram-сессия и медиафайлы
переживают рестарты. Создайте сервер в разделе **Облачные серверы** (выберите образ «Docker» из
маркетплейса, чтобы Docker был предустановлен), зайдите по SSH и повторите шаги из
[Быстрого старта через Docker](#быстрый-старт-через-docker-рекомендуется) выше. Настройте домен и
реверс-прокси (nginx/Caddy) с HTTPS перед портом `8000` — так же, как описано для systemd-варианта
в [Управлении сервисами](#управление-сервисами).

**Вариант 2: App Platform (Timeweb Cloud Apps) — без администрирования сервера.** PaaS от Timeweb
разворачивает приложение прямо из `Dockerfile` этого репозитория (в нём уже указан `EXPOSE 8000`),
даёт бесплатный домен, HTTPS и отдельную управляемую PostgreSQL. Готовой ссылки для деплоя в один
клик у них нет (в отличие от Railway) — вместо этого: подключите свой форк на GitHub в разделе
**Apps**, разверните из Dockerfile, укажите путь для health check — `/healthz`, создайте
управляемую Postgres и пропишите строку подключения в `DATABASE_URL`, затем задайте
`APP_SECRET_KEY`, `ADMIN_PASSWORD` и остальное из [Конфигурации](#конфигурация-env) как переменные
окружения и запустите деплой.

> **Важный нюанс:** файловая система контейнера в App Platform не гарантированно переживает
> редеплой — значит, Telegram-сессия и медиафайлы в очереди на публикацию могут быть потеряны при
> редеплое, и после этого нужно будет заново войти через **Settings → Telegram-аккаунт**. Это
> нормально, если вы деплоите один раз и редко туда возвращаетесь; у варианта с VPS такого
> ограничения нет.

### Установка вручную

Хотите запускать напрямую через свой Python/PostgreSQL вместо Docker? Ниже — как это сделать.
...
> Используете Docker? См. [Быстрый старт через Docker](#быстрый-старт-через-docker-рекомендуется)
> — достаточно `docker compose up -d` / `docker compose down`. Разделы ниже — для ручной установки
> (без Docker).
...
Откройте админку через SSH-туннель (`ssh -L 8000:127.0.0.1:8000 user@server`) или настройте nginx
как reverse proxy с HTTPS.
...
3. **Targets** → добавьте целевые каналы с `chat_id`, проверьте кнопкой `Test`; задайте расписание
   публикаций.
...
6. **Posts** → просматривайте новые посты, запускайте AI-генерацию, редактируйте и публикуйте.
   Используйте `Ctrl+P` для предпросмотра в стиле Telegram.
7. **Stats** → смотрите воронку, график публикаций по дням, топ источников.
8. **Settings** → подключите Telegram-аккаунт (QR-код или телефон+код), подстройте пороги
   дедупликации, промпты, интервал сбора, уведомления оператору.

---

## Миграции БД

Миграции применяются автоматически при каждом старте через `ALTER TABLE … ADD COLUMN` в
try/except. Ничего запускать вручную не нужно.
...
Все Telegram ID-колонки (`telegram_message_id`, `telegram_grouped_id`, `last_message_id`,
`telegram_channel_id`) автоматически расширяются до `BIGINT` — Telegram использует 64-битные
числа, которые не помещаются в стандартный `INTEGER`.
...
- Семантическая дедупликация требует ~220 МБ под модель и заметно нагружает CPU. На слабых VPS
  рекомендуется держать выключенной.
...
Идеи, баг-репорты, правки в документации и pull request'ы — всё это приветствуется.

- Нашли баг или есть идея? Заведите [issue](../../issues/new/choose).
- Готовы прислать код? Смотрите [CONTRIBUTING.md](CONTRIBUTING.md) (на английском) — там настройка
  окружения и процесс review PR. Сами issue и PR можно писать по-русски.
- Нашли уязвимость? Пожалуйста, следуйте [SECURITY.md](SECURITY.md) вместо публичного issue.

Проект следует [Contributor Covenant](CODE_OF_CONDUCT.md) — кодексу поведения участников
сообщества.
