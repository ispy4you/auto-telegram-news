...
- Интеграция с **Timeweb AI Gateway** (OpenAI-совместимый endpoint) — легко подключить любой
  OpenAI-совместимый API.
- Правила для AI хранятся в базе и правятся прямо в админке; там же кнопка «Проверить на свежей новости»
  и справка о том, что уходит в модель помимо правил.
...
- **Своё медиа**: загрузить фото или видео (JPEG/PNG/WebP/MP4), поменять порядок в альбоме,
  убрать картинку с водяным знаком. Загруженные файлы лежат на диске контейнера и не переживают
  редеплой — панель говорит об этом прямо, а не публикует молча без них.
- **Вкладка «Генерация»**: вставить свой текст и получить пост по тем же правилам — для новостей,
  которые пришли мимо источников.
